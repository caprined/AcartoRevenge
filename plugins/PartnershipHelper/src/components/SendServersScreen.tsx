...
export default function SendServersScreen({ onClose, targetChannelId }: Props) {
        ...
        <View style={rst.backdrop} pointerEvents="box-none">
            ...
            <Animated.View style={[rst.sheet, { transform: [{ translateY: slide }] }]}>
                ...
                <View style={rst.header}>
                    <Text style={rst.title}>Wyślij serwery</Text>
                ...
                <FlatList
                    ...
                    data={servers}
                    keyExtractor={(s) => s.id}
                    ListEmptyComponent={<Text style={rst.empty}>Brak dodanych serwerów. Dodaj je w "Zarządzaj serwerami".</Text>}
                    renderItem={({ item }) => (
                        <View style={rst.row}>
                            <GuildIcon guildId={item.guildId} size={36} />
                            <Text style={rst.rowName} numberOfLines={1}>{item.name}</Text>
                            <Pressable
                                style={[rst.sendBtn, !item.enabled && rst.sendBtnDisabled]}

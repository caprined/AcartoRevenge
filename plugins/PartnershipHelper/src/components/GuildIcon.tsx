...
export default function GuildIcon({ guildId, size = 44 }: { guildId: string | null; size?: number }) {
    if (!guildId) {
        return (
            <View style={[st.fallback, { width: size, height: size, borderRadius: size >= 40 ? 14 : 8, backgroundColor: "#4e5058" }]}>
                <Text style={{ color: "#fff", fontSize: 16 }}>?</Text>
            </View>
        );
    }

    const g = GuildStore?.getGuild?.(String(guildId));
